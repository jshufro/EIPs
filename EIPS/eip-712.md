...
* `uint256 chainId` the [EIP-155](./eip-155.md) chain id. The user-agent _should_ refuse signing if it does not match the currently active chain.
...
**Alternative 3**: ABIv2 function signatures modified to be 256-bit. While this captures type info, it does not capture any of the semantics other than the function. This is already causing a practical collision between [EIP-20](./eip-20.md)'s and [EIP-721](./eip-721.md)'s `transfer(address,uint256)`, where in the former the `uint256` refers to an amount and the latter to a unique id. In general ABIv2 favors compatibility where a hashing standard should prefer incompatibility.
